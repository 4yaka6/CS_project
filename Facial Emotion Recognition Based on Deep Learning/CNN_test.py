...
from CNN_train import EmotionClassifier
...
def get_test_loader():
    ...
    return test_loader, test_dataset.classes  # 返回 DataLoader 和类标签

# 验证模型
def validate_model(model, validation_loader, class_names):
    all_labels = []
    all_preds = []

    # 使用 no_grad() 来进行验证时不计算梯度
    with torch.no_grad():
...
if __name__ == '__main__':
    # 加载模型
    model_path = 'resultsave/cnnloss0.12acc5766.pth'  # 选择适当的模型路径
